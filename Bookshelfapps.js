const filterTabButtons = document.querySelectorAll(".box_item .btn");
const addBookForm = document.querySelector("#addBookForm");
const allBooksDivContainer = document.querySelector("#allBooks");
const unFinishedReadBooksContainer = document.querySelector("#unFinishedReadBooks");
const finishedReadBooksContainer = document.querySelector("#finishedReadBooks");
...
const inputKeywordBook = document.querySelector("#searchBook");
...
function addBookHandler(event) {
  ...
  const isCompleted = addBookForm["inputBookIsCompleted"].checked;
  ...
  const invalidFeedbackTitle = addBookForm["bookTitle"]
    .closest(".form-control")
    .querySelector(".invalid-feedback");

  if (isBookTitleExist(title)) {
    addBookForm["bookTitle"].classList.add("invalid");
  ...
  populateBooks();
  addBookForm["bookTitle"].classList.remove("invalid");
...
function populateBooks() {
  ...
  unFinishedReadBooksContainer.innerHTML = "";
  finishedReadBooksContainer.innerHTML = "";
  ...
  if (bookList.length === 0 && keyword) {
    ...
    finishedReadBooksContainer.innerHTML = renderNoResultFound();
    unFinishedReadBooksContainer.innerHTML = renderNoResultFound();
    return;
  }
  ...
  bookList.forEach((book) => {
    ...
    if (book.isCompleted) {
      finishedReadBooksContainer.appendChild(createBookCard(book));
    } else {
      unFinishedReadBooksContainer.appendChild(createBookCard(book));
    }
  });
}
